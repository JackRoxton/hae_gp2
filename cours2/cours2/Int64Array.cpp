...
int Int64Array::bSearch(int dtmin, int dtmax, int64_t key) { //MARCHE PO
	int len = curSize - dtmin - (curSize - dtmax);
	int piv = len / 2;
	...
	if (val < key) {
		return bSearch(piv+1,len+1, key);
	}
	else {
	return bSearch(0,len-1, key);
	}
};
